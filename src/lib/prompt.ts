...
const ART_STYLE =
  ...
  "顔はシンプルな漫画表現に整理し、丸みのある輪郭、整理された目元、少ない陰影、フラットで明るい塗りにする。" +
  "頭はやや大きめ、体は少し小さめのLINEスタンプ向けデフォルメ比率にする。" +
...
const RESEMBLANCE_TEXT: Record<string, string> = {
  close:
    "本人にできるだけ寄せた似顔絵にする（ただし実写ではなく手描き風キャラクター）。写真の質感は使わず、特徴だけを拾う。",
  caricature:
    "本人の特徴をいかした親しみやすい似顔絵。顔の輪郭・髪型・目元・口元の特徴は残しつつ、写真ではなく2D手描きキャラクターとして整理する。写実よりもLINEスタンプらしいかわいい似顔絵を優先する。",
  atmosphere:
...
export function buildCharacterPortraitPrompt(answers: WizardAnswers): string {
  return [
    ...
    "顔は写真の精密再現ではなく、丸い輪郭・整理された目鼻口・少ない影・フラットな肌色で、LINEスタンプ向けの似顔絵アバターにする。",
    "頭をやや大きく、肩から上をコンパクトにしたかわいいデフォルメ比率。写実的な首・肩・肌の陰影は避ける。",
